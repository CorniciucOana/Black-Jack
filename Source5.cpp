...
void readFromDatabase(int& game, player& firstPlayer, player& secondPlayer){
	database.close();
...
void printToDatabase(int game, player firstPlayer, player* secondPlayer){
...
int main(){
	...
	while (!exit){
		...
		cout << endl << "--Menu--" << endl;
		cout << "1: Print player money" << endl;
		cout << "2: Update player details" << endl;
		cout << "3: Play black jack" << endl;
		cout << "0: Exit" << endl << endl;
		cin >> command;
		switch (command){
		case 1:
			cout << "Write player name: ";
			cin.get();  cin.getline(tempName, 255);
			if (!database){ cout << tempName << " isn't at the table right now!" << endl; }
			else {
				readFromDatabase(game, tempPlayer1, tempPlayer2);
				if (strcmp(tempName, tempPlayer1.name) == 0){
					cout << tempName << " has " << tempPlayer1.money << " dollars." << endl;
					continue;
				}
				if (strcmp(tempName, tempPlayer2.name) == 0){
					cout << tempName << " has " << tempPlayer2.money << " dollars." << endl;
					continue;
				}
				cout << tempName << " isn't at the table right now!" << endl;
				continue;
			}
			break;
		case 2:
			...
			if (!database){ cout << tempName << " isn't at the table right now!" << endl; }
				...
				readFromDatabase(game, tempPlayer1, tempPlayer2);
				if (strcmp(tempName, tempPlayer1.name) == 0){
					cout << "Write the players new name: ";
					cin.getline(tempPlayer1.name, 255);
					cout << "Write the players new ammunt: ";
					cin >> tempPlayer1.money;
					printToDatabase(game, tempPlayer1, &tempPlayer2);
					continue;
				}
				if (strcmp(tempName, tempPlayer2.name) == 0){
					cout << "Write the players new name: ";
					cin.getline(tempPlayer2.name, 255);
					cout << "Write the players new ammunt: ";
					cin >> tempPlayer2.money;
					printToDatabase(game, tempPlayer1, &tempPlayer2);
					continue;
				}
				cout << tempName << " isn't at the table right now!" << endl;
			...
			if (database){
				...
				if (game == 1)
				{
					...
					if (game)
					{
						...
						database.close();
						database.open(FILE_NAME, ios::out | ios::trunc);
						database.close();
						cout << "All data has been cleared! You can start fresh at the table!";
					}
				}
				else
				{
